<?php
if ( ! defined( 'ABSPATH' ) ) { exit; } class GFIPPANELSMS_Pro_Configurations { public static function construct() { if ( defined( 'RG_CURRENT_PAGE' ) && in_array( RG_CURRENT_PAGE, array( 'admin-ajax.php' ) ) ) { add_action( 'wp_ajax_gf_select_ippanelsms_form', array( __CLASS__, 'select_forms_ajax' ) ); add_action( 'wp_ajax_nopriv_gf_select_ippanelsms_form', array( __CLASS__, 'select_forms_ajax' ) ); } } public static function configuration() { wp_register_style( 'gform_admin_sms', GFCommon::get_base_url() . '/css/admin.css' ); wp_print_styles( array( 'jquery-ui-styles', 'gform_admin_sms', 'wp-pointer' ) ); ?>

        <div class="wrap gforms_edit_form gf_browser_gecko">

			<?php
 $id = ! rgempty( "ippanelsms_setting_id" ) ? rgpost( "ippanelsms_setting_id" ) : absint( rgget( "id" ) ); $config = empty( $id ) ? array( "is_active" => true, "meta" => array() ) : GFIPPANELSMS_Pro_SQL::get_feed( $id ); $get_feeds = GFIPPANELSMS_Pro_SQL::get_feeds(); $form_name = ''; $_get_form_id = ! empty( $config["form_id"] ) ? $config["form_id"] : rgget( 'fid' ); foreach ( (array) $get_feeds as $get_feed ) { if ( $get_feed['id'] == $id ) { $form_name = $get_feed['form_title']; } } ?>

            <h2 class="gf_admin_page_title"><?php _e( "SMS configuration for forms", "GF_IPPanel" ) ?>

				<?php if ( ! empty( $_get_form_id ) ) { ?>
                    <span class="gf_admin_page_subtitle">
					<span class="gf_admin_page_formid"><?php echo sprintf( __( "Feed ID: %s", "GF_IPPanel" ), $id ) ?></span>
					<span
                            class="gf_admin_page_formname"><?php echo sprintf( __( "Form Name: %s", "GF_IPPanel" ), $form_name ) ?></span>
				</span>
				<?php } ?>

            </h2>
            <a class="button add-new-h2" href="admin.php?page=gf_settings&subview=gf_sms_pro"
               style="margin:8px 9px;"><?php _e( "SMS General settings", "GF_IPPanel" ) ?></a>

			<?php
 $settings = GFIPPANELSMS_Pro::get_option(); $is_OK = ( ! empty( $settings["ws"] ) && $settings["ws"] != 'no' ); if ( $is_OK ) { echo '<div style="display:inline-table;margin-top:7px !important">'; GFIPPANELSMS_Pro::show_credit( $settings["cr"], true ); echo '</div>'; } else { wp_die(); } if ( ! rgempty( "gf_ippanelsms_submit" ) ) { check_admin_referer( "update", "gf_ippanelsms_feed" ); $config["form_id"] = absint( rgpost( "gf_ippanelsms_form" ) ); $config["meta"]["from"] = rgpost( "gf_ippanelsms_from" ); $config["meta"]["to"] = rgpost( "gf_ippanelsms_to" ); $config["meta"]["to_c"] = rgpost( "gf_ippanelsms_to_c" ); $config["meta"]["when"] = rgpost( "gf_ippanelsms_when" ); $config["meta"]["message"] = rgpost( "gf_ippanelsms_message" ); $config["meta"]["message_c"] = rgpost( "gf_ippanelsms_message_c" ); $config["meta"]["gf_sms_change_code"] = rgpost( 'gf_sms_change_code' ); $config["meta"]["gf_change_code_type"] = rgpost( "gf_change_code_type" ); $config["meta"]["gf_code_static"] = rgpost( "gf_code_static" ); $config["meta"]["gf_code_dyn"] = rgpost( "ippanelsms_gf_code_dyn" ); $config["meta"]["gf_sms_is_gateway_checked"] = rgpost( 'gf_sms_is_gateway_checked' ); $config["meta"]["customer_field_clientnum"] = rgpost( "ippanelsms_customer_field_clientnum" ); $config["meta"]["adminsms_conditional_enabled"] = rgpost( 'gf_adminsms_conditional_enabled' ); $config["meta"]["adminsms_conditional_type"] = rgpost( 'gf_adminsms_conditional_type' ); $config["meta"]["adminsms_conditional_field_id"] = rgpost( 'gf_adminsms_conditional_field_id' ); $config["meta"]["adminsms_conditional_operator"] = rgpost( 'gf_adminsms_conditional_operator' ); $config["meta"]["adminsms_conditional_value"] = rgpost( 'gf_adminsms_conditional_value' ); $config["meta"]["clientsms_conditional_enabled"] = rgpost( 'gf_clientsms_conditional_enabled' ); $config["meta"]["clientsms_conditional_type"] = rgpost( 'gf_clientsms_conditional_type' ); $config["meta"]["clientsms_conditional_field_id"] = rgpost( 'gf_clientsms_conditional_field_id' ); $config["meta"]["clientsms_conditional_operator"] = rgpost( 'gf_clientsms_conditional_operator' ); $config["meta"]["clientsms_conditional_value"] = rgpost( 'gf_clientsms_conditional_value' ); $safe_data = array(); foreach ( $config["meta"] as $key => $val ) { if ( in_array( $key, array( 'adminsms_conditional_operator', 'clientsms_conditional_operator' ) ) ) { $safe_data[ $key ] = $val; } else if ( in_array( $key, array( 'message', 'message_c' ) ) ) { $safe_data[ $key ] = wp_kses( $val, array( 'br' => array() ) ); } else if ( ! is_array( $val ) ) { $safe_data[ $key ] = sanitize_text_field( $val ); } else { $safe_data[ $key ] = array_map( 'sanitize_text_field', $val ); } } $config["meta"] = $safe_data; $id = GFIPPANELSMS_Pro_SQL::update_feed( $id, $config["form_id"], $config["is_active"], $config["meta"] ); if ( ! headers_sent() ) { wp_redirect( admin_url( 'admin.php?page=gf_ippanelsms&view=edit&id=' . $id . '&updated=true' ) ); exit; } ?>

                <div class="updated fade"
                     style="padding:6px"><?php echo sprintf( __( "Feed Updated. %sback to list%s", "GF_IPPanel" ), "<a href='?page=gf_ippanelsms'>", "</a>" ) ?></div>

				<?php
 } $_get_form_id = ! empty( $config["form_id"] ) ? $config["form_id"] : rgget( 'fid' ); if ( rgget( 'updated' ) == 'true' ) { $id = empty( $id ) && isset( $_GET['id'] ) ? rgget( 'id' ) : $id; ?>
                ...
                <div id="gf_form_toolbar">
                    <ul id="gf_form_toolbar_links">
						...
						<?php
 $menu_items = apply_filters( 'gform_toolbar_menu', GFForms::get_toolbar_menu_items( $_get_form_id ), $_get_form_id ); echo GFForms::format_toolbar_menu_items( $menu_items ); ?>

                        <li class="gf_form_switcher">
                            <label for="export_form"><?php _e( 'Select a feed', 'GF_IPPanel' ) ?></label>
							<?php
 $feeds = GFIPPANELSMS_Pro_SQL::get_feeds(); ?>
                            <select name="form_switcher" id="form_switcher"
                                    onchange="GF_SwitchForm(jQuery(this).val());">
                                <option value=""><?php _e( 'Switch SMS feed', 'GF_IPPanel' ) ?></option>
								<?php foreach ( $feeds as $feed ) { $selected = $feed["id"] == $id ? "selected='selected'" : ""; ?>
                                    <option
                                            value="<?php echo $feed["id"] ?>" <?php echo $selected ?> ><?php echo sprintf( __( 'Form:%s (Feed:%s)', 'GF_IPPanel' ), $feed["form_title"], $feed["id"] ) ?></option>
								<?php } ?>
                            </select>
                        </li>
                    </ul>
            ...
            <div id="gform_tab_group" class="gform_tab_group vertical_tabs">
				<?php if ( ! empty( $_get_form_id ) ) { ?>
                    <ul id="gform_tabs" class="gform_tabs">
						...
						<?php
 $get_form = GFFormsModel::get_form_meta( $_get_form_id ); $current_tab = rgempty( 'subview', $_GET ) ? 'settings' : rgget( 'subview' ); $current_tab = ! empty( $current_tab ) ? $current_tab : ''; $setting_tabs = GFFormSettings::get_tabs( $get_form['id'] ); if ( ! empty( $current_tab ) ) { foreach ( $setting_tabs as $tab ) { $query = array( 'page' => 'gf_edit_forms', 'view' => 'settings', 'subview' => $tab['name'], 'id' => $get_form['id'] ); $url = add_query_arg( $query, admin_url( 'admin.php' ) ); echo $tab['name'] == 'sms' ? '<li class="active">' : '<li>'; ?>
                        ...
                        <div id="form_settings" class="gform_panel gform_panel_form_settings">
                            <h3>
								<span>
									...
									<?php _e( "General configuration", "GF_IPPanel" ); ?>
								</span>
                            </h3>
                            <form method="post" action="" id="gform_form_settings">

								<?php wp_nonce_field( "update", "gf_ippanelsms_feed" ) ?>

                                <input type="hidden" name="ippanelsms_setting_id" value="<?php echo $id ?>"/>
                                <table class="gforms_form_settings" cellspacing="0" cellpadding="0">
                                    ...
                                    <tr id="ippanelsms_form_container">
                                        <th>
											<?php _e( "Select form", "GF_IPPanel" ); ?>
                                        </th>
                                        <td>
                                            <select id="gf_ippanelsms_form" name="gf_ippanelsms_form"
                                                    onchange="SelectFormAjax(jQuery(this).val());">
                                                <option
                                                        value=""><?php _e( "Please select a form", "GF_IPPanel" ); ?> </option>
												<?php
 $forms = RGFormsModel::get_forms(); foreach ( (array) $forms as $form ) { $selected = absint( $form->id ) == $_get_form_id ? "selected='selected'" : ""; ?>
                                                    <option
                                            ...
                                            <img src="<?php echo esc_url( GFCommon::get_base_url() ) ?>/images/spinner.gif"
                                                 id="ippanelsms_wait" style="display: none;"/>
                                        </td>
                                    </tr>
                                    </tbody>
                                </table>
                                ...
                                <table class="gforms_form_settings"
                                       id="ippanelsms_field_group" <?php echo empty( $_get_form_id ) ? "style='display:none;'" : "" ?>
                                       cellspacing="0" cellpadding="0">
                                    <tbody>
                                    <tr>
                                        <th>
											<?php _e( "Sender Number", "GF_IPPanel" ); ?>
                                        </th>
                                        <td>
                                            <select id="gf_ippanelsms_from" name="gf_ippanelsms_from">
                                                <option value=""><?php _e( "Select Sender Number", "GF_IPPanel" ); ?></option>
												<?php
 $sender_num = isset( $settings["from"] ) ? $settings["from"] : ''; if ( $sender_num == '' || strpos( $settings["from"], ',' ) === false ) { if ( $sender_num and $sender_num != '' ) { $selected = ( isset( $config["meta"]["from"] ) && $sender_num == $config["meta"]["from"] ) ? "selected='selected'" : ""; ?>
                                                        <option
                                                                value="<?php echo $sender_num ?>" <?php echo $selected ?> ><?php echo $sender_num ?></option>
														<?php
 } } else { unset( $sender_num ); $sender_nums = array(); if ( ! empty( $settings["from"] ) ) { $sender_nums = explode( ',', $settings["from"] ); } foreach ( (array) $sender_nums as $sender_num ) { $selected = ( isset( $config["meta"]["from"] ) && $sender_num == $config["meta"]["from"] ) ? "selected='selected'" : ""; ?>
                                                        <option
                                                                value="<?php echo $sender_num ?>" <?php echo $selected ?> ><?php echo $sender_num ?></option>
														<?php
 } } ?>
                                    ...
                                    <tr>
                                        <th>
											<?php _e( "Payment Gateway integration", "GF_IPPanel" ); ?>
                                        </th>
                                        <td>
                                            ...
                                            <label style="font-family:tahoma !important;"
                                                   for="gf_sms_is_gateway_checked"><?php _e( "Check if only your form is connected to payment gateways.", "GF_IPPanel" ); ?></label><br/>
                                            <table cellspacing="0" cellpadding="0">

                                                <tr>
                                                    <td>
                                                        <div id="gf_sms_is_gateway_checked_box" <?php echo ! rgar( $config['meta'], 'gf_sms_is_gateway_checked' ) ? 'style="display:none"' : ''; ?>>
                                                            <p><?php _e( "Sending time configuration : ", "GF_IPPanel" ) ?></p>
                                                            <select id="gf_ippanelsms_when" name="gf_ippanelsms_when">
                                                                <option
                                                                        value="send_immediately" <?php echo ( isset( $config["meta"]["when"] ) && "send_immediately" == $config["meta"]["when"] ) ? "selected='selected'" : ""; ?> ><?php _e( "Immediately after form submission", "GF_IPPanel" ); ?> </option>
                                                                <option
                                                                        value="after_pay" <?php echo ( isset( $config["meta"]["when"] ) && "after_pay" == $config["meta"]["when"] ) ? "selected='selected'" : ""; ?> ><?php _e( "After Payment(All payment statuses)", "GF_IPPanel" ); ?> </option>
                                                                <option
                                                                        value="after_pay_success" <?php echo ( isset( $config["meta"]["when"] ) && "after_pay_success" == $config["meta"]["when"] ) ? "selected='selected'" : ""; ?> ><?php _e( "After Successful Payment", "GF_IPPanel" ); ?> </option>
                                                            </select>
                                                            <p class="description"><?php _e( '<strong>Note : </strong>Your Payment gateway must be standard. it is required to use "gform_post_payment_status" action.', 'GF_IPPanel' ) ?></p>
                                                        </div>
                                                    </td>
                                                </tr>
                                            </table>
                                    ...
                                    <tr>
                                        <th>
											<?php _e( "Change Country Code", "GF_IPPanel" ); ?>
                                        </th>
                                        <td>
                                            ...
                                            <label for="gf_sms_change_code"
                                                   style="font-family:tahoma !important;"><?php _e( "Check if you want to change default country code.", "GF_IPPanel" ); ?></label><br/>
                                            <table cellspacing="0" cellpadding="0">
                                                <tr>
                                                    <td>
                                                        ...
                                                        <div id="gf_sms_change_code_box" <?php echo isset( $config['meta'] ) && ! rgar( $config['meta'], 'gf_sms_change_code' ) ? 'style="display:none"' : ''; ?>>
                                                            ...
                                                            <span id="ippanelsms_gf_code_dyn_div" <?php echo isset( $config['meta'] ) && rgar( $config['meta'], 'gf_change_code_type' ) == 'dyn' ? '' : 'style="display:none"'; ?>>
																<?php
 if ( ! empty( $_get_form_id ) ) { $form_meta = RGFormsModel::get_form_meta( $_get_form_id ); echo ! empty( $form_meta ) ? self::get_country_code( $form_meta, $config ) : ''; } ?>
																</span>

                                                            <p class="description"><?php _e( '<strong>Note : </strong>You can change the default country code. but If entered mobile phone number was international format, this country code will be effectless.', 'GF_IPPanel' ); ?></p>
                                    ...
                                    <tr>
                                        <td colspan="2">
                                            <h4 class="gf_settings_subgroup_title">
												<?php _e( "Admin SMS Configuration : ", "GF_IPPanel" ); ?>
												<?php _e( "Leave blank for unsending.", "GF_IPPanel" ); ?>
                                            </h4>
                                        </td>
                                    </tr>
                                    ...
                                    <tr>
                                        <th>
											<?php _e( "Admin Numbers", "GF_IPPanel" ); ?>
                                        </th>
                                        <td>
                                            <input type="text" class="fieldwidth-1" name="gf_ippanelsms_to"
                                            ...
                                            <span
                                                    class="description"><?php _e( "Separate with commas (,). Format with a '+' and country code e.g., +16175551212", "GF_IPPanel" ) ?></span>
                                        </td>
                                    </tr>

                                    <tr>
                                        <th>
											<?php _e( "Admin Message Body", "GF_IPPanel" ); ?>
                                        </th>
                                        <td>
                                            <select id="gf_ippanelsms_message_variable_select"
                                                    onchange="InsertVariable('gf_ippanelsms_message');">
												<?php if ( ! empty( $_get_form_id ) ) { $form_meta = RGFormsModel::get_form_meta( $_get_form_id ); echo ! empty( $form_meta ) ? self::get_form_fields_merge( $form_meta ) : ''; } ?>
                                            </select>
                                            <br/>
                                            <textarea id="gf_ippanelsms_message" name="gf_ippanelsms_message"
                                    ...
                                    <tr id="gf_adminsms_conditional_option">
                                        <th>
											<?php _e( "Conditional Logic", "GF_IPPanel" ); ?>
                                        </th>
                                        <td>
                                            ...
                                            <label style="font-family:tahoma,serif !important;"
                                                   for="gf_adminsms_conditional_enabled"><?php _e( "Enable Condition for Admin", "GF_IPPanel" ); ?></label><br/>
                                                ...
                                                <tr>
                                                    <td>
                                                        <div id="gf_adminsms_conditional_container" <?php echo ! rgar( $config['meta'], 'adminsms_conditional_enabled' ) ? "style='display:none'" : "" ?>>
                                                            ...
                                                            <span><?php _e( "Send SMS to Admin if ", "GF_IPPanel" ) ?></span>

                                                            <select name="gf_adminsms_conditional_type">
                                                                <option value="all" <?php echo rgar( $config['meta'], 'adminsms_conditional_type' ) == 'all' ? "selected='selected'" : "" ?>><?php _e( "All", "GF_IPPanel" ) ?></option>
                                                                <option value="any" <?php echo rgar( $config['meta'], 'adminsms_conditional_type' ) == 'any' ? "selected='selected'" : "" ?>><?php _e( "Any", "GF_IPPanel" ) ?></option>
                                                            </select>
                                                            <span><?php _e( "of the following match:", "GF_IPPanel" ) ?></span>

															<?php
 if ( ! empty( $config["meta"]["adminsms_conditional_field_id"] ) ) { $admin_conditions = $config["meta"]["adminsms_conditional_field_id"]; if ( ! is_array( $admin_conditions ) ) { $admin_conditions = array( '1' => $admin_conditions ); } } else { $admin_conditions = array( '1' => '' ); } if ( ! empty( $config["meta"]["adminsms_conditional_value"] ) ) { $admin_condition_values = $config["meta"]["adminsms_conditional_value"]; if ( ! is_array( $admin_condition_values ) ) { $admin_condition_values = array( '1' => $admin_condition_values ); } } else { $admin_condition_values = array( '1' => '' ); } if ( ! empty( $config["meta"]["adminsms_conditional_operator"] ) ) { $admin_condition_operators = $config["meta"]["adminsms_conditional_operator"]; if ( ! is_array( $admin_condition_operators ) ) { $admin_condition_operators = array( '1' => $admin_condition_operators ); } } else { $admin_condition_operators = array( '1' => 'is' ); } ksort( $admin_conditions ); foreach ( $admin_conditions as $i => $value ):?>
                                                                ...
                                                                <div class="gf_adminsms_conditional_div"
                                                                    ...
                                                                    <select class="gf_adminsms_conditional_operator"
                                                                            ...
                                                                            title="">
                                                                        <option value="is"><?php _e( "is", "GF_IPPanel" ) ?></option>
                                                                        <option value="isnot"><?php _e( "is not", "GF_IPPanel" ) ?></option>
                                                                        <option value=">"><?php _e( "greater than", "GF_IPPanel" ) ?></option>
                                                                        <option value="<"><?php _e( "less than", "GF_IPPanel" ) ?></option>
                                                                        <option value="contains"><?php _e( "contains", "GF_IPPanel" ) ?></option>
                                                                        <option value="starts_with"><?php _e( "starts with", "GF_IPPanel" ) ?></option>
                                                                        <option value="ends_with"><?php _e( "ends with", "GF_IPPanel" ) ?></option>
                                                                    </select>
                                                            ...
                                                            <div id="gf_adminsms_conditional_message"
																...
																<?php _e( "To create a registration condition, your form must have a field supported by conditional logic.", "GF_IPPanel" ) ?>
                                                            </div>
                                    ...
                                    <tr>
                                        <td colspan="2">
                                            <h4 class="gf_settings_subgroup_title">
												<?php _e( "User SMS Configuration :", "GF_IPPanel" ); ?>
												<?php _e( "Leave them blank for unsending.", "GF_IPPanel" ); ?>
                                            </h4>
                                        </td>
                                    </tr>
                                    <tr>
                                        <th>
											<?php _e( "Phone Number Mapping", "GF_IPPanel" ); ?>
                                        </th>
                                        <td id="ippanelsms_customer_field">
											<?php
 if ( ! empty( $_get_form_id ) ) { $form_meta = RGFormsModel::get_form_meta( $_get_form_id ); echo ! empty( $form_meta ) ? self::get_client_information( $form_meta, $config ) : ''; } ?>
                                        </td>
                                    </tr>
                                    <tr>
                                        <th>
											<?php _e( "Extra Numbers", "GF_IPPanel" ); ?>
                                        </th>
                                        <td>
                                            <input type="text" class="fieldwidth-1" name="gf_ippanelsms_to_c"
                                    ...
                                    <tr>
                                        <th>
											<?php _e( "User SMS Body", "GF_IPPanel" ); ?>
                                        </th>
                                        <td>
                                            <select id="gf_ippanelsms_message_c_variable_select"
                                                    onchange="InsertVariable('gf_ippanelsms_message_c');">
												<?php
 if ( ! empty( $_get_form_id ) ) { $form_meta = RGFormsModel::get_form_meta( $_get_form_id ); echo ! empty( $form_meta ) ? self::get_form_fields_merge( $form_meta ) : ''; } ?>
                                            </select>
                                            <br/>
                                            <textarea id="gf_ippanelsms_message_c" name="gf_ippanelsms_message_c"
                                    ...
                                    <tr id="gf_clientsms_conditional_option">
                                        ...
                                        <td>
                                            ...
                                            <label style="font-family:tahoma,serif !important;"
                                                   for="gf_clientsms_conditional_enabled"><?php _e( "Enable Condition for User", "GF_IPPanel" ); ?></label><br/>
                                                ...
                                                <tr>
                                                    <td>
                                                        <div id="gf_clientsms_conditional_container" <?php echo ! rgar( $config['meta'], 'clientsms_conditional_enabled' ) ? "style='display:none'" : "" ?>>
                                                            ...
                                                            <span><?php _e( "Send SMS to User if ", "GF_IPPanel" ) ?></span>

                                                            <select name="gf_clientsms_conditional_type">
                                                                <option value="all" <?php echo rgar( $config['meta'], 'clientsms_conditional_type' ) == 'all' ? "selected='selected'" : "" ?>><?php _e( "All", "GF_IPPanel" ) ?></option>
                                                                <option value="any" <?php echo rgar( $config['meta'], 'clientsms_conditional_type' ) == 'any' ? "selected='selected'" : "" ?>><?php _e( "Any", "GF_IPPanel" ) ?></option>
                                                            </select>
                                                            <span><?php _e( "of the following match:", "GF_IPPanel" ) ?></span>

															<?php
 if ( ! empty( $config["meta"]["clientsms_conditional_field_id"] ) ) { $client_conditions = $config["meta"]["clientsms_conditional_field_id"]; if ( ! is_array( $client_conditions ) ) { $client_conditions = array( '1' => $client_conditions ); } } else { $client_conditions = array( '1' => '' ); } if ( ! empty( $config["meta"]["clientsms_conditional_value"] ) ) { $client_condition_values = $config["meta"]["clientsms_conditional_value"]; if ( ! is_array( $client_condition_values ) ) { $client_condition_values = array( '1' => $client_condition_values ); } } else { $client_condition_values = array( '1' => '' ); } if ( ! empty( $config["meta"]["clientsms_conditional_operator"] ) ) { $client_condition_operators = $config["meta"]["clientsms_conditional_operator"]; if ( ! is_array( $client_condition_operators ) ) { $client_condition_operators = array( '1' => $client_condition_operators ); } } else { $client_condition_operators = array( '1' => 'is' ); } ksort( $client_conditions ); foreach ( $client_conditions as $i => $value ):?>
                                    ...
                                    <tr>
                                        <td>
                                            <input type="submit" class="button-primary gfbutton"
                                                   name="gf_ippanelsms_submit"
                                                   value="<?php _e( "Update SMS Configuration", "GF_IPPanel" ); ?>"/>
                                        </td>
                                    </tr>
                                    </tbody>
                                </table>
	...
	display: inline-block;
        ...
        <script type="text/javascript">
            ...
            jQuery(document).ready(function ($) {
                ...
                $(document.body).on('click', 'input[name="gf_change_code_type"]', function () {
                    if ($('input[name="gf_change_code_type"]:checked').val() === 'dyn') {
                        $("#ippanelsms_gf_code_dyn_div").show("slow");
                        $("#ippanelsms_gf_code_static").hide("slow");
                    }
                    else {
                        $("#ippanelsms_gf_code_dyn_div").hide("slow");
                        $("#ippanelsms_gf_code_static").show("slow");
                    }
            ...
            function SelectFormAjax(formId) {
                if (!formId) {
                    jQuery("#ippanelsms_field_group").slideUp();
                } else {
                    jQuery("#ippanelsms_wait").show();
                    jQuery("#ippanelsms_field_group").slideUp();
                    jQuery.post(ajaxurl, {
                            action: "gf_select_ippanelsms_form",
                            gf_select_ippanelsms_form: "<?php echo wp_create_nonce( "gf_select_ippanelsms_form" ) ?>",
                            form_id: formId,
                        ...
                        function (data) {
                            ...
                            jQuery("#gf_ippanelsms_message_variable_select").html(fields);
                            jQuery("#gf_ippanelsms_message_c_variable_select").html(fields);
                            jQuery("#ippanelsms_customer_field").html(data["customer_field"]);
                            jQuery("#ippanelsms_gf_code_dyn_div").html(data["gf_code"]);
                            ...
                            jQuery("#ippanelsms_field_group").slideDown();
                            jQuery("#ippanelsms_wait").hide();
                        }, "json"
                    );
                }
            }
            ...
            function GetConditionalFieldValues(input, fieldId, selectedOperator, selectedValue, labelMaxCharacters, index) {
                ...
                if (selectedOperator == '' || selectedOperator == 'is' || selectedOperator == 'isnot') {
                    if (field["type"] == "post_category" && field["displayAllCategories"]) {
                        str += '<?php $dd = wp_dropdown_categories( array( "class" => "condition_field_value", "orderby" => "name", "id" => "gf_dropdown_cat_id", "name" => "gf_dropdown_cat_name", "hierarchical" => true, "hide_empty" => 0, "echo" => false ) ); echo str_replace( "\n", "", str_replace( "'", "\\'", $dd ) ); ?>';
                ...
                if (is_text) {
                    ...
                    str += "<input type='text' class='condition_field_value' style='padding:3px' placeholder='<?php _e( "Enter value", "GF_IPPanel" ); ?>' id='" + input + "_value' name='" + name + "' value='" + selectedValue + "'>";
                }
                return str;
            }
            ...
            function GF_SwitchForm(id) {
                ...
                if (id.length > 0) {
                    ...
                    if (is_form_settings)
                        new_query = "page=gf_ippanelsms&view=edit&id=" + id;
...
jQuery(function($){
			...
			var targettextarea = $(this).parents('td').find('textarea');
					...
					success: function (json) {
						...
						var message = '';							
							  if(obj.status == 0){
								...
								var pvars = obj.vars;
								var output = '<p>لطفاً پس از کلیک بر روی ثبت الگو در کادر پیامک، جلوی هر یک از متغیرهای پترن که در کادر درج شده ، یکی از برچسب های ادغام را درج نمایید.</p>';
								...
								pvars.forEach(function(value, index, array){
									...
									targettextarea.css('cssText','direction: ltr !important; width: 400px;height:100px').val(patternoutput);
		...
		<?php
 } public static function get_client_information( $form, $config ) { $form_fields = self::get_client_form_fields( $form ); $str = ""; $selected_field = $config && ! empty( $config["meta"]["customer_field_clientnum"] ) ? $config["meta"]["customer_field_clientnum"] : ""; $str .= self::get_mapped_fields( "customer_field_clientnum", $selected_field, $form_fields, 'true' ); return $str; } public static function get_country_code( $form, $config ) { $form_fields = self::get_client_form_fields( $form ); $str = ""; $selected_field = $config && ! empty( $config["meta"]["gf_code_dyn"] ) ? $config["meta"]["gf_code_dyn"] : ""; $str .= self::get_mapped_fields( "gf_code_dyn", $selected_field, $form_fields, 'false' ); return $str; } public static function get_mapped_fields( $variable_name, $selected_field, $fields, $empty ) { $field_name = "ippanelsms_" . $variable_name; $str = "<select name=\"$field_name\" id=\"$field_name\">"; $str .= $empty == 'true' ? "<option value=\"\"></option>" : ""; foreach ( (array) $fields as $field ) { $field_id = $field[0]; $field_label = esc_html( GFCommon::truncate_middle( $field[1], 40 ) ); $selected = $field_id == $selected_field ? "selected='selected'" : ""; $str .= "<option value=\"$field_id\" " . $selected . ">" . $field_label . "</option>"; } $str .= "</select>"; return $str; } public static function get_client_form_fields( $form ) { $fields = array(); if ( is_array( $form["fields"] ) ) { foreach ( (array) $form["fields"] as $field ) { if ( isset( $field["inputs"] ) && is_array( $field["inputs"] ) ) { foreach ( (array) $field["inputs"] as $input ) { if ( ! ( GFCommon::is_pricing_field( $field["type"] ) || ( $field["type"] == 'total' ) ) ) { $fields[] = array( $input["id"], GFCommon::get_label( $field, $input["id"] ) ); } } } else if ( ! rgar( $field, 'displayOnly' ) ) { if ( ! ( GFCommon::is_pricing_field( $field["type"] ) || ( $field["type"] == 'total' ) ) ) { $fields[] = array( $field["id"], GFCommon::get_label( $field ) ); } } } } return $fields; } public static function get_form_fields_merge( $form ) { $str = "<option value=''>" . __( "Merge Tags", "gravityforms" ) . "</option>"; $required_fields = array(); $optional_fields = array(); $pricing_fields = array(); foreach ( (array) $form["fields"] as $field ) { if ( $field["displayOnly"] ) { continue; } $input_type = RGFormsModel::get_input_type( $field ); if ( $field["isRequired"] ) { switch ( $input_type ) { case "name" : if ( $field["nameFormat"] == "extended" ) { $prefix = GFCommon::get_input( $field, $field["id"] + 0.2 ); $suffix = GFCommon::get_input( $field, $field["id"] + 0.8 ); $optional_field = $field; $optional_field["inputs"] = array( $prefix, $suffix ); $optional_fields[] = $optional_field; unset( $field["inputs"][0] ); unset( $field["inputs"][3] ); } $required_fields[] = $field; break; default: $required_fields[] = $field; } } else { $optional_fields[] = $field; } if ( GFCommon::is_pricing_field( $field["type"] ) ) { $pricing_fields[] = $field; } } if ( ! empty( $required_fields ) ) { $str .= "<optgroup label='" . __( "Required form fields", "gravityforms" ) . "'>"; foreach ( (array) $required_fields as $field ) { $str .= self::get_fields_options( $field ); } $str .= "</optgroup>"; } if ( ! empty( $optional_fields ) ) { $str .= "<optgroup label='" . __( "Optional form fields", "gravityforms" ) . "'>"; foreach ( (array) $optional_fields as $field ) { $str .= self::get_fields_options( $field ); } $str .= "</optgroup>"; } if ( ! empty( $pricing_fields ) ) { $str .= "<optgroup label='" . __( "Pricing form fields", "gravityforms" ) . "'>"; foreach ( (array) $pricing_fields as $field ) { $str .= self::get_fields_options( $field ); } $str .= "</optgroup>"; } $str .= "<optgroup label='" . __( "Other", "gravityforms" ) . "'>
					<option value='{payment_gateway}'>" . __( "Payment Gateway / Method", "GF_IPPanel" ) . "</option>
			...
			</optgroup>"; return apply_filters( 'gravity_sms_pro_merge_tags_list', $str, $form ); } public static function get_fields_options( $field, $max_label_size = 100 ) { $str = ""; if ( is_array( $field["inputs"] ) ) { foreach ( (array) $field["inputs"] as $input ) { $str .= "<option value='{" . esc_attr( GFCommon::get_label( $field, $input["id"] ) ) . ":" . $input["id"] . "}'>" . esc_html( GFCommon::truncate_middle( GFCommon::get_label( $field, $input["id"] ), $max_label_size ) ) . "</option>"; } } else { $str .= "<option value='{" . esc_html( GFCommon::get_label( $field ) ) . ":" . $field["id"] . "}'>" . esc_html( GFCommon::truncate_middle( GFCommon::get_label( $field ), $max_label_size ) ) . "</option>"; } return $str; } public static function select_forms_ajax() { check_ajax_referer( "gf_select_ippanelsms_form", "gf_select_ippanelsms_form" ); $form_id = intval( rgpost( "form_id" ) ); $form = RGFormsModel::get_form_meta( $form_id ); $fields = self::get_form_fields_merge( $form ); $customer_field = self::get_client_information( $form, '' ); $gf_code = self::get_country_code( $form, '' ); $result = array( "form" => $form, "fields" => $fields, "customer_field" => str_replace( "'", "\'", $customer_field ), "gf_code" => str_replace( "'", "\'", $gf_code ) ); if ( ob_get_length() ) { ob_clean(); } header( 'Content-Type: application/json' ); wp_die( GFCommon::json_encode( $result ) ); exit; } } 